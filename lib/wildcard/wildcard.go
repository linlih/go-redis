...
func (i *item) contains(c byte) bool {
	// 如果不是范围集合的话直接查看该元素是否在集合中
	if i.typeCode == setSymbol {
		_, ok := i.set[c]
		return ok
	} else if i.typeCode == rangSymbol { // 范围集合的话，查看
		if _, ok := i.set[c]; ok {
			return true
		}
		var (
		...
		for k := range i.set { // 这个地方得到 set 集合中最小和最大的元素
			if min > k {
				min = k
			}
			if max < k {
		...
		return c >= min && c <= max // 判断是否在集合中
	} else {
		_, ok := i.set[c] // 如果是 ^a  表示的是 negSymbol 不是 a 元素开头的就返回成功
		return !ok
	}
}
...
// CompilePattern 将模式串编译成 items 元素
